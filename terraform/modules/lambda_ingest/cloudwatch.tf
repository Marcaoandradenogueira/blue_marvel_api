...
resource "aws_cloudwatch_event_target" "check_lambda_every_time_interval" {
  ...
  target_id   = "lambda_function_ingest"
  arn         = aws_lambda_function.lambda_function_ingest.arn
}

resource "aws_lambda_permission" "allow_cloudwatch_to_call_check_lambda" {
  statement_id = "AllowExecutionFromCloudWatch"
  action       = "lambda:InvokeFunction"
  function_name = aws_lambda_function.lambda_function_ingest.function_name
