...
from pyspark.sql.functions import col, max, to_timestamp, date_format, from_unixtime, unix_timestamp, explode, arrays_zip
...
class DeltaLakeProcessing():
    ...
    def __init__(
        ...
        self.raw_prefix_path = raw_prefix_path
    ...
    def read_from_processing_stage(self, format_file: str = 'json'):
        ...
        try:
            path = self.raw_prefix_path + 'marvel_lake_raw/day-summary/' +  self.information+ '/'
            df_raw = self.spark.read.format(format_file).load(path)


            df_data = df_raw.select("data.*")
            df_upsert = df_data.withColumn(self.information, explode(arrays_zip("results")))\
                .select(f'{self.information}.results.*')

            w = Window.partitionBy(self.primary_keys)

            # Get ts_ms with max timestamp using primary keys as reference
            self.df_upsert  = df_upsert.withColumn('tmp_ts_ms_max', max('modified').over(w))\
                .where(col('modified') == col('tmp_ts_ms_max'))\
                .drop('tmp_ts_ms_max').drop_duplicates()
                
            self.logger.info("Characters Files Loaded.")

        # If table doesn't exists in Debizium pass to next step in ENR.
        except pyspark.sql.utils.AnalysisException as error:
            if 'Path does not exist' in error.args[0]:
    ...
    def process_upsert_and_delete(self):
        ...
        path = f"{self.staged_prefix_path}{self.information}"
        table_name = self.information
    ...
    def process_curated(self):
        ...
        try:
            ...
            glue_table_name = self.information
...
if __name__ == '__main__':
    ...
    delta_lake = DeltaLakeProcessing(
        raw_prefix_path=params.get("raw_prefix_path"),
